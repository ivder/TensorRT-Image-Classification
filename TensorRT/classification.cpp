...
classifier_ctx* ClassificationTensorRT::classifier_initialize(std::string model_file, std::string trained_file, std::string mean_file, std::string label_file)
{
	try
	{
		int device_count;
...
const char* ClassificationTensorRT::classifier_classify(classifier_ctx* ctx, cv::Mat img)
{
	try
	{
		if (img.empty())
		...
		const auto p = max_element(predictions.begin(), predictions.end(), [](const auto& lhs, const auto& rhs) { return lhs.second < rhs.second; });
...
void ClassificationTensorRT::classifier_destroy(classifier_ctx* ctx)
{
	delete ctx;
}